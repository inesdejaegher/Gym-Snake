...
def logbook_simulation(file_path, episode, n_drugs_consumed, n_food_consumed, total_reward, snake_length, steps, loop):
    ...
    with open(file_path, mode='a', newline='') as csvfile:
        ...
        if not file_exists:
            writer.writerow(['Episode', 'Drugs_Consumed', 'Food_Consumed', 'Preference_Ratio', 'Total_Reward', 'Snake_Length', 'Steps', 'Loop'])
            
        # Write the data for the current episode
        writer.writerow([episode, n_drugs_consumed, n_food_consumed, ratio, total_reward, snake_length, steps, loop])
        
    return ratio, total_reward, snake_length, steps
