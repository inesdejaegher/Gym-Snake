...
def extract_drug_reward(q_table_path):
    ...
    Example:
    "DRUG_R1_NO_GROWTH_Q_EPXXX_TIME_XXX.pkl"
    returns 1.
    """
    q_table_name = os.path.basename(q_table_path)
    match = re.search(r"DRUG_R(\d+)_NO_GROWTH", q_table_name)
...
def find_q_tables(q_table_dir):
    ...
    """
    pattern = os.path.join(q_table_dir, "DRUG_R*_GROWTH10_Q_EP*_TIME_*.pkl")
...
def evaluate_q_table(q_table_path):
    ...
    condition_name = f"DRUG_R{drug_reward}_YES_GROWTH"
...
if __name__ == "__main__":
    ...
    for q_table_path in q_table_paths:
        try:
            csv_path = evaluate_q_table(q_table_path)
