...
def extract_drug_reward(q_table_path):
    ...
    Example:
    "DRUG_R10_ENG_NO_PEN_Q_EP15K_TIME_XXX.pkl"
    returns 10.
    """
    q_table_name = os.path.basename(q_table_path)
    match = re.search(r"DRUG_R(\d+)_ENG_NO_PEN", q_table_name)
...
def find_q_tables(q_table_dir):
    ...
    """
    pattern = os.path.join(q_table_dir, "DRUG_R*_ENG_NO_PEN_Q_EP*_TIME_*.pkl")
...
def evaluate_q_table(q_table_path):
    ...
    condition_name = f"DRUG_R{drug_reward}_ENG_NO_PEN"
    ...
    csv_dir = os.path.join(os.path.dirname(__file__), "..", "Results", "Drugs_Energy_No_Penalty")
