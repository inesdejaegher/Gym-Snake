...
def extract_drug_reward(q_table_path):
    ...
    Example:
    q_table_drug_reward_25_no_energy_penalty_EP_5000_TIME_04_05_2026_17-49-47.pkl
    returns 25.
    """
    q_table_name = os.path.basename(q_table_path)
    match = re.search(r"drug_reward_(\d+)_no_energy_penalty", q_table_name)
...
def find_q_tables(q_table_dir):
    """
    Finds all no-energy-penalty drug Q-tables and sorts them by drug reward.
    """
    pattern = os.path.join(q_table_dir, "q_table_drug_reward_*_no_energy_penalty_EP_*_TIME_*.pkl")
...
def evaluate_q_table(q_table_path):
    ...
    condition_name = f"drug_reward_{drug_reward}_no_energy_penalty"

    # ----- STORAGE FOLDER FOR RESULTS -----
    csv_name = f"Evaluation_Results_logbook_{q_table_name.replace('.pkl', '.csv')}"
    csv_dir = os.path.join(os.path.dirname(__file__), "..", "Results", "Drugs_No_Energy_Penalty")
...
if __name__ == "__main__":
    ...
    if len(q_table_paths) == 0:
        raise FileNotFoundError(f"No no-energy-penalty drug Q-tables found in: {q_table_dir}")
