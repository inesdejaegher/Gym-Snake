...
q_table_name = f"BASE_ENERGY_Q_EP15k_TIME_{datetime.datetime.now().strftime('%d_%m_%Y_%H-%M-%S')}.pkl"
...
def run_training():
    ...
    for episode in range(episodes):
        ...
        if (episode + 1) % 100 == 0:
            ...
            logging.info(
                f"Baseline Energy | Episode {episodes_done}/{episodes} "
...
if __name__ == "__main__":
    logging.info("Starting 1 baseline energy training with 1 parallel worker...")

    with ProcessPoolExecutor(max_workers=1) as executor:
        future_to_name = {
            executor.submit(run_training): "Baseline Energy"
        }
